...
export function InstagramReports({ data }) {
    if (!data) return <EmptyState title="Export Reports" description="Generate and download custom PDF/CSV reports of your Instagram performance." icon={FileText} actionLabel="Generate Report" />;
          ...
          <CardContent className="space-y-6">
            <div className="flex flex-col sm:flex-row items-end gap-4 p-4 border border-white/10 rounded-lg bg-white/[0.02]">
              <div className="w-full sm:flex-1 space-y-2">
                ...
                <input type="date" className="w-full bg-[#0B1121] border border-white/10 rounded-md px-3 py-2 text-sm text-slate-300 focus:outline-none focus:border-[#E1306C]" />
              </div>
              <div className="w-full sm:flex-1 space-y-2">
                <label className="text-xs font-medium text-slate-400">Report Type</label>
                <select className="w-full bg-[#0B1121] border border-white/10 rounded-md px-3 py-2 text-sm text-slate-300 focus:outline-none focus:border-[#E1306C]">
              ...
              <div className="flex gap-3 w-full sm:w-auto">
                <button className="flex-1 sm:flex-none px-4 py-2 bg-[#E1306C] hover:bg-pink-600 text-white text-sm font-medium rounded-md transition-colors whitespace-nowrap">
                  Export CSV
                </button>
