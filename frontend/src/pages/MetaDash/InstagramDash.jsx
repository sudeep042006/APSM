...
import { IG_NAV } from "./MetaInnerSidebar";
...
export default function InstagramDash() {
  ...
  // ── Check Instagram connection status via /auth/status ───────────
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        const res = await metaApi.getAuthStatus();
        const statusArray = res?.status ?? [];
        const igStatus = statusArray.find(s => s.platform === "instagram");
        if (igStatus?.connected && !igStatus?.isExpired) {
          setIsConnected(true);
        } else {
        ...
        console.error("Error checking Instagram auth status:", err);
        setIsConnected(false);
      } finally {
        setIsLoading(false);
      }
  ...
  const fetchInstagramData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      if (!disconnectedRef.current) {
        setAnalyticsData(res?.instagram ?? {});
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (isConnected) fetchInstagramData();
  }, [isConnected, fetchInstagramData]);

  const handleRefresh = () => fetchInstagramData(true);

  // ── Initiate Instagram OAuth redirection ──
  // Redirects the browser to the backend OAuth initialization route for Instagram,
  ...
  const handleInstagramConnect = () => {
    ...
    window.location.href = `${import.meta.env.VITE_BASE_URL || "http://localhost:5000"}/auth/instagram?token=${token}`;
  };
  ...
  const handleRevoke = async () => {
    ...
    try {
      ...
      console.error("Instagram revoke error:", err);
    } finally {
      setIsConnected(false);
      setAnalyticsData(null);
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] w-full overflow-hidden bg-slate-50 dark:bg-[#0B1121] text-slate-900 dark:text-slate-100 transition-colors duration-200 -m-6">
      ...
      <div className="sticky top-0 z-30 bg-white/95 dark:bg-[#0B1121]/90 backdrop-blur-md border-b border-slate-200 dark:border-white/10 px-6 flex flex-col gap-0">
        ...
        <div className="flex items-center justify-between py-3">
          ...
          <div className="flex items-center gap-1">
            {/* Facebook tab — navigates to FacebookDash */}
            <NavLink
            ...
            {/* Instagram tab — always active on this route */}
            <NavLink
      ...
      <div className={`flex-1 overflow-y-auto ${!isConnected ? "flex flex-col" : ""}`}>
        <div className={`p-6 flex flex-col gap-6 ${!isConnected ? "flex-1 justify-center" : ""}`}>
          {!isConnected ? (
            /* ── Instagram Connection Card Empty State ── */
            /* Centers the ConnectCard in the remaining height of the viewport */
            <div className="flex-1 flex items-center justify-center">
              <ConnectCard
                icon={<Instagram className="h-8 w-8" />}
                cardTitle="Connect Instagram Profile"
                cardDescription="Connect your Instagram account to view reach, engagement, followers, and detailed visual performance graphs."
            ...
            <EmptyDataState platform="Instagram" />
          ) : (
