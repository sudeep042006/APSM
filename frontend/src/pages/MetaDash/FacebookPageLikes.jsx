...
export function FacebookPageLikes({ data }) {
  if (!data) return <EmptyState title="Page Likes Dashboard" description="Track your page like growth over time and see what campaigns are driving followers." icon={ThumbsUp} actionLabel="Promote Page" />;

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <KpiCard label="Total Gained" value={data.gained} change={15.2} changeLabel="vs last month" icon={TrendingUp} color="bg-emerald-500/10 text-emerald-500" />
        <KpiCard label="Total Lost" value={data.lost} change={-4.1} changeLabel="vs last month" icon={TrendingDown} color="bg-red-500/10 text-red-500" />
        <KpiCard label="Net Growth" value={data.net} change={11.3} changeLabel="vs last month" icon={Users} color="bg-blue-500/10 text-blue-500" />
      </div>
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.followerGrowthTimeline} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="fbGrowthTrend" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="gained" stroke="#1877F2" strokeWidth={3} fill="url(#fbGrowthTrend)" name="Gained" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.followerGrowthTimeline} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="gained" name="Followers Gained" fill="#1877F2" radius={[4, 4, 0, 0]} barSize={20} />
