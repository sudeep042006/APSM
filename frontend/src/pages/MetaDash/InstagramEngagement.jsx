...
export function InstagramEngagement({ data }) {
  if (!data) return <EmptyState title="Engagement Metrics Pending" description="Once your audience starts interacting with your content, interaction trends will appear here." icon={Heart} />;

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <KpiCard label="Total Likes" value={data.kpis.totalLikes} change={5.4} changeLabel="vs last month" icon={Heart} color="bg-pink-500/10 text-pink-500" />
        <KpiCard label="Total Comments" value={data.kpis.totalComments} change={2.1} changeLabel="vs last month" icon={MessageCircle} color="bg-purple-500/10 text-purple-500" />
        <KpiCard label="Total Saves" value={data.kpis.totalSaves} change={12.4} changeLabel="vs last month" icon={Bookmark} color="bg-indigo-500/10 text-indigo-500" />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.engagementTrend} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="igEngTrend" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor={IG_PINK} stopOpacity={0.4} />
                  ...
                  <Area type="monotone" dataKey="total" name="Total Engagement" stroke={IG_PINK} strokeWidth={2} fill="url(#igEngTrend)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="bg-[#161B22] border-white/5 text-white xl:col-span-1">
          <CardHeader>
            <CardTitle className="text-sm font-medium text-slate-200">Interaction Breakdown</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.reactionTypes} layout="vertical" margin={{ top: 20, right: 30, left: 10, bottom: 5 }}>
                  ...
                  <YAxis type="category" dataKey="name" stroke="#64748b" fontSize={12} tickLine={false} axisLine={false} width={70} />
                  <Tooltip content={<DarkTooltip />} cursor={{ fill: "#ffffff05" }} />
                  <Bar dataKey="value" name="Count" fill={IG_PINK} radius={[0, 4, 4, 0]} barSize={20}>
